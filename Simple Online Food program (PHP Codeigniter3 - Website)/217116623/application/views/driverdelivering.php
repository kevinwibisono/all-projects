...
<body>
	...
	<div id='content' style="clear:left; background-color: white; opacity: 0.75; text-align: center; width: 1000px; margin: auto">
		...
		<h3>You're delivering:</h3>
		<?php foreach($htrans as $h){
				if($h->email_driver == $current_email && $h->status == "Pengiriman"){?>
				<div class='trans'>
				<?php 
					if($h->tglpengiriman != null){
						echo "Since: ".$h->tglpengiriman;
						echo "<hr>";
					}
				?>
				...
				<?php
					echo form_open('mainpage/pengiriman');
					echo form_hidden('HiddenIDTrans', $h->id_trans);
					echo form_submit('', 'Delivered');
					echo form_close();
				?>
				 </div>
				<?php }} ?>
