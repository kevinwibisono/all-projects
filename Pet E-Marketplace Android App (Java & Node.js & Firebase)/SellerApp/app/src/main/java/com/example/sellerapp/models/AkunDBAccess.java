...
public class AkunDBAccess {
    ...
    public AkunDBAccess(Context app) {
        db = RoomDB.getInstance(app);
    }
    ...
    public Task<DocumentSnapshot> getAccByEmail(String email){
        //karena menggunakan get(), maka hanya akan query 1x saja
...
//    };
    ...
    public Task<Void> register(String email, String password){
        saveAccount = new Akun(email, "", password);
        ...
        akun.put("saldo", 0);
        akun.put("penjual", true);

        return firebaseDb.collection("akun").document(email)
                .set(akun);
    }

    public Task<Void> assignName(Akun savedAcc, String name){
        saveAccount = savedAcc;
        saveAccount.setNama(name);

        Map<String, Object> akun = new HashMap<>();
        akun.put("nama", name);

        return firebaseDb.collection("akun").document(savedAcc.getEmail())
                .set(akun, SetOptions.merge());
    }
