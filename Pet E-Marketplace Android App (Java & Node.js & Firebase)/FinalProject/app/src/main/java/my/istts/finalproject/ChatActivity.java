...
import my.istts.finalproject.adapters.ChatAdapter;
import my.istts.finalproject.databinding.ActivityChatBinding;
...
import my.istts.finalproject.interfaces.chatListItem;
import my.istts.finalproject.interfaces.chatListOrder;
import my.istts.finalproject.interfaces.chatListPictures;
...
public class ChatActivity extends AppCompatActivity implements chatListPictures, chatListItem, chatListOrder, chatChooseProduct, chatChooseHotel, chatChooseOrder {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ChatAdapter adapter = new ChatAdapter(new ChatAdapter.onShowImage() {
            ...
            public void showProduct(String id) {
                Intent productIntent = new Intent(ChatActivity.this, PetProductActivity.class);
                productIntent.putExtra("id_produk", id);
                startActivity(productIntent);
            }
        }, new ChatAdapter.onShowHotel() {
            @Override
            public void showHotel(String id) {
                Intent roomIntent = new Intent(ChatActivity.this, HotelDetailActivity.class);
                roomIntent.putExtra("id_kamar", id);
                startActivity(roomIntent);
            }
        }, new ChatAdapter.onShowOrder() {
            @Override
            public void showOrder(String id, int tipe) {
                if (tipe < 3) {
        ...
        }, new ChatAdapter.onShowAdopt() {
            @Override
            public void showPetAdopt(String id) {
                Intent detailIntent = new Intent(ChatActivity.this, PetAdoptDetailActivity.class);
                detailIntent.putExtra("id_pet", id);
        ...
        viewModel.getChatVMs().observe(this, vms -> {
            adapter.setCurrent_hp(viewModel.getNoHp());
    ...
    private void checkChatIntent(){
        if(getIntent().hasExtra("id_item")){
            int tipe = getIntent().getIntExtra("tipe", 0);
            if(tipe == 0){
                chooseProduct(getIntent().getStringExtra("id_item"));
            }
            else if(tipe == 1){
                chooseOrder(getIntent().getStringExtra("id_item"));
            }
            else if(tipe == 2){
                chooseHotel(getIntent().getStringExtra("id_item"));
            }
            else{
                chooseAdoption(getIntent().getStringExtra("id_item"));
            }
            toggleCardSend(tipe+1);
        }
    }
    ...
    private void toggleCardSend(int showNumber){
        ...
        LinearLayout selectedAdopt = findViewById(R.id.chatSendAdopt);
        ...
        if(showNumber == 0){
            ...
            selectedAdopt.setVisibility(View.GONE);
        }
            ...
            selectedRoom.setVisibility(View.GONE);
            selectedOrder.setVisibility(View.GONE);
            selectedAdopt.setVisibility(View.VISIBLE);
        }
    }
    ...
    private void chooseAdoption(String id_adopt){
        viewModel.setChosenAdopt(id_adopt);
        ImageView ivAdopt = findViewById(R.id.chatSendAdoptPic);
        viewModel.getChosenItemPic().observe(this, picUrl -> {
            if(!picUrl.equals("")){
                Glide.with(this).load(picUrl).into(ivAdopt);
            }
        });
